...
# Load the gold dataset from Hugging Face
dataset = load_dataset("allenai/scifact", "claims")

# Evaluate the IR system on each claim
k = 50  # Number of top documents to retrieve
total_map = 0
total_mrr = 0
num_claims = 0  
...
for claim_pair, claim_vector in claim_embeddings.items():
    ...
    if not gold_labels:
        continue  # Skip if no relevant gold labels found
